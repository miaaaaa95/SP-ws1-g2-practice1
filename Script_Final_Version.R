...
for (i in seq_len(length(sentence))) {
  if (length(which(sentence[i] == uni_capital_words) == 0)) ##Exclude words that are never capitalised in the text
    ...
    if (prob_i > 0.5) {                                    
      sentence_capital[i] <- firstupper(sentence[i])     ## words that most often start with a capital letter in the main text, also start with a capital letter in generated text
